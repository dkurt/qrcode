                 ...
                 std::vector<int> &xs)
{
   ...
   while (current_ind < length) {
      ...
      else if (xs.size() == 0 && row[current_ind - 1] == 0) {
         xs.push_back(current_ind - 1);
         int current_count = 1;
         ++current_ind;
         while (row[current_ind] == row[current_ind - 1] && current_ind < length) {
            ++current_ind;
                ...
                const cv::Point &topRight, const cv::Point &bottomLeft)
{
   ...
   srcPoints[0] = topRight;
   dstPoints[0] = cv::Point2f(17, 3);
   srcPoints[1] = topLeft;
   dstPoints[1] = cv::Point2f(3, 3);
   srcPoints[2] = bottomLeft;
   dstPoints[2] = cv::Point2f(3, 17);
   srcPoints[3] = bottomRight;
   dstPoints[3] = cv::Point2f(17, 17);
